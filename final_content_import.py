...
def convert_page_to_specific_type(page_id, model_class, additional_fields=None):
    """Convert a generic Page to a specific page type"""
    with connection.cursor() as cursor:
        # Get the app label and model name
        app_label = model_class._meta.app_label
        model_name = model_class._meta.model_name
        
        # First, insert into the specific model's table
        cursor.execute(f"""
            INSERT INTO {app_label}_{model_name} (page_ptr_id)
            VALUES (%s)
...
def ensure_index_pages():
    ...
    if not faq_index:
        ...
        if existing_faq:
            if not isinstance(existing_faq.specific, FAQIndexPage):
                print(f"Converting existing FAQ page (id: {existing_faq.id}) to FAQIndexPage...")
                convert_page_to_specific_type(
                    existing_faq.id, 
                    FAQIndexPage,
                ...
                faq_index = existing_faq.specific
        else:
    ...
    if not encyclopedia_index:
        ...
        if existing_encyclopedia:
            if not isinstance(existing_encyclopedia.specific, EncyclopediaIndexPage):
                ...
                encyclopedia_index = existing_encyclopedia.specific
