...
class NewsletterSignupViewTest(BasePublicSiteTestCase, FormTestMixin):
    ...
    def test_newsletter_creates_crm_contact(self):
        ...
        mock_contact_class.objects.get_or_create.return_value = (mock_contact_instance, True)
        
        with patch.dict('sys.modules', {'crm': Mock(), 'crm.models': Mock(Contact=mock_contact_class), 'crm.models.choices': Mock(ContactType=Mock(), ContactStatus=Mock())}):
            data = self.create_test_newsletter_data()
            response = self.submit_form('/newsletter/signup/', data, follow=False)
...
class OnboardingFormViewTest(BasePublicSiteTestCase, FormTestMixin):
    ...
    def test_onboarding_form_validation(self):
        ...
        data = self.create_test_onboarding_data()
        data['initial_investment'] = '10000'  # Below minimum
        data['accredited_investor'] = False  # Not accredited
    ...
    def test_onboarding_thank_you_page(self):
        ...
        self.assertIn('Application Received', response.content.decode())
        self.assertIn('Thank You for Your Application', response.content.decode())
