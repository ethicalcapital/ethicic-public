                ...
                if hasattr(post, 'body') and post.body:
                    ...
                    for old_url, new_url in url_mappings.items():
                        if old_url in body_text:
                            ...
                            self.stdout.write(f"Updated {old_url} -> {new_url} in: {post.title}")
                            changes_made += 1
                    ...
                    if body_text != original_text and not dry_run:
                        ...
                        def update_blocks(blocks):
                            for block in blocks:
                                if block.get('type') == 'paragraph' and 'value' in block:
                                    original_value = block['value']
                                    new_value = original_value
                                    for old_url, new_url in url_mappings.items():
                                        new_value = new_value.replace(old_url, new_url)
                                    block['value'] = new_value
                                elif block.get('type') == 'rich_text' and 'value' in block:
                                    original_value = block['value']
                                    new_value = original_value
                                    for old_url, new_url in url_mappings.items():
                                        new_value = new_value.replace(old_url, new_url)
                        ...
                        post.body = StreamValue(post.body.stream_block, body_data, is_lazy=True)
                        post.save()
            ...
            for page in other_pages:
                if hasattr(page, 'body') and page.body:
                    ...
                    for old_url, new_url in url_mappings.items():
                        if old_url in body_text:
                            body_text = body_text.replace(old_url, new_url)
                            self.stdout.write(f"Updated {old_url} -> {new_url} in page: {page.title}")
                            changes_made += 1
                    ...
                    if body_text != original_text and not dry_run:
                        ...
                        try:
                            ...
                            self.stdout.write(f"Could not update page {page.title} - complex body structure")
