    ...
    def handle(self, *args, **options):
        ...
        for post in blog_posts:
            if hasattr(post, 'body') and post.body:
                ...
                if 'precisefp' in body_text.lower():
                    self.stdout.write(f"PreciseFP found in: {post.title} (ID: {post.id})")
                    self.stdout.write(f"URL: {post.get_url()}")
                    
                if '/performance/' in body_text:
                    self.stdout.write(f"Performance ref found in: {post.title} (ID: {post.id})")
                    self.stdout.write(f"URL: {post.get_url()}")
        ...
        for page in other_pages:
            if hasattr(page, 'body') and page.body:
                ...
                if 'precisefp' in body_text.lower():
                    self.stdout.write(f"PreciseFP found in page: {page.title} (ID: {page.id})")
                    self.stdout.write(f"URL: {page.get_url()}")
                    
                if '/performance/' in body_text:
                    self.stdout.write(f"Performance ref found in page: {page.title} (ID: {page.id})")
                    self.stdout.write(f"URL: {page.get_url()}")
