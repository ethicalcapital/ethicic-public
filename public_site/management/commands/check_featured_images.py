...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        if image_count > 0:
            ...
            try:
                self.stdout.write(f"  - File exists: {sample_image.file.storage.exists(sample_image.file.name)}")
        ...
        for post in posts_with_images[:3]:  # Check first 3
            ...
            try:
                ...
                self.stdout.write(f"  - Rendition file exists: {rendition.file.storage.exists(rendition.file.name)}")
            except Exception as e:
