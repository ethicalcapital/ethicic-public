...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        self.stdout.write(f"Found {len(posts_with_references)} posts with investvegan.org references")
        
        if not posts_with_references:
            self.stdout.write(self.style.SUCCESS("No posts found with investvegan.org references."))
            return
            ...
            for post in posts_with_references:
                ...
                for image_url in image_urls_found:
                    if image_url in updated_content or image_url in updated_body:
                        post_changes.append(f"IMAGE FOUND (needs manual review): {image_url}")
        ...
        if image_urls_found:
            ...
            self.stdout.write("These images may need to be re-uploaded to ethicic.com's media system.")
