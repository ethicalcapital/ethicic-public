...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        if not monthly_returns:
            self.stdout.write(
                self.style.ERROR('No valid data found in CSV')
            )
            return
    ...
    def parse_csv(self, csv_file):
        ...
        for line_num, line in enumerate(lines):
            ...
            if len(parts) > 1 and ('MSCI ACWI TR' in parts[1] or 'Benchmark TR' in parts[1]):
                ...
                if current_year and strategy_row and benchmark_row:
                    ...
                    for i, month in enumerate(months):
                        if i < len(strategy_row) and i < len(benchmark_row):
                            ...
                            if (strategy_val and benchmark_val and 
                                '%' in strategy_val and '%' in benchmark_val and
                                strategy_val != '' and benchmark_val != ''):
...
if __name__ == '__main__':
    # Example usage
    print("Example usage:")
    print("python manage.py import_performance_csv --csv-file growth_performance.csv --strategy-title 'Growth Strategy'")