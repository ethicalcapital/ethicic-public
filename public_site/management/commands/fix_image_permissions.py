        ...
        try:
            ...
            for group in groups:
                ...
                if image_perms.exists():
                    self.stdout.write(f"  Group '{group.name}' has {image_perms.count()} image permissions")
                else:
                    self.stdout.write(f"  Group '{group.name}' has NO image permissions")
            ...
            if users.count() > 0:
                for user in users:
                    if user.is_superuser:
                        ...
                        self.stdout.write(f"  User '{user.username}' is in {user_groups.count()} groups")
