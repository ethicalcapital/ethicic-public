...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        for page in all_pages:
            ...
            self.stdout.write(
                f"{indent}- {page.title} (ID: {page.id}, Type: {page_type}, Status: {status})"
            )
        ...
        for site in sites:
            self.stdout.write(
                f"Site: {site.hostname} (Root Page: {site.root_page.title})"
            )

        # Check page permissions
        self.stdout.write("\n=== Page Permissions ===")
        page_permissions = Permission.objects.filter(
            content_type__app_label="wagtailcore", content_type__model="page"
        )
        ...
        for model in page_models:
            ...
            if count > 0:
                for page in model.objects.all():
                    self.stdout.write(
                        f"  - {page.title} (ID: {page.id}, Live: {page.live}, URL: {page.url})"
                    )
        ...
        if not public_pages.exists():
            self.stdout.write(
                self.style.WARNING("No public_site pages found in database!")
            )
        ...
        if orphaned.exists():
            self.stdout.write(
                self.style.WARNING(
                    f"Found {orphaned.count()} orphaned pages at root level"
                )
            )
        ...
        for page in public_pages[:5]:  # Check first 5 pages
            ...
            self.stdout.write(
                f"- {specific_page.title}: Has content_panels: {has_content_panels}"
            )
