...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        self.stdout.write(self.style.SUCCESS("🔍 Verifying Wagtail URL Structure Fix"))
        ...
        try:
            ...
            if homepage.slug == "":
                ...
                self.stdout.write(
                    f"  ❌ Homepage slug is '{homepage.slug}' (should be empty)"
                )
                success = False
            ...
            if homepage.url_path == "/":
                ...
                self.stdout.write(
                    f"  ❌ Homepage URL path is '{homepage.url_path}' (should be '/')"
                )
                success = False
        ...
        if "homepage" in locals():
            ...
            for child in child_pages:
                ...
                if child.url_path == expected_url:
                    ...
                    self.stdout.write(
                        f"  ❌ {child.title}: {child.url_path} (expected {expected_url})"
                    )
                    success = False
        ...
        if pages_with_home.exists():
            self.stdout.write(
                f"  ❌ Found {pages_with_home.count()} pages with /home/ in URL:"
            )
        ...
        if success:
            self.stdout.write(
                self.style.SUCCESS(
                    "🎉 All checks passed! Wagtail URL structure is fixed."
                )
            )
            ...
            self.stdout.write(
                "✅ All child pages serve at proper URLs (/about/, /contact/, etc.)"
            )
            ...
            self.stdout.write(
                "3. Update any hardcoded URLs in templates or documentation"
            )
        else:
            self.stdout.write(
                self.style.ERROR(
