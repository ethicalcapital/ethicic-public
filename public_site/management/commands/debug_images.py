...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        if os.path.exists(settings.MEDIA_ROOT):
            self.stdout.write(f"✅ Media root exists")
            if os.access(settings.MEDIA_ROOT, os.W_OK):
                self.stdout.write(f"✅ Media root is writable")
            else:
                self.stdout.write(f"❌ Media root is not writable")
        else:
            self.stdout.write(f"❌ Media root does not exist")
        ...
        for img in images:
            ...
            if img.file:
                ...
                if os.path.exists(file_path):
                    ...
                    self.stdout.write(f"    ❌ File missing from disk")
                
                # Check renditions
                renditions = img.renditions.all()[:3]
                self.stdout.write(f"    📐 Renditions: {len(renditions)} total, showing first 3:")
                ...
                for rendition in renditions:
                    ...
                    if rendition.file and os.path.exists(rend_path):
                        ...
                        self.stdout.write(f"      ✅ {rendition.filter_spec}: {rend_path} ({size} bytes)")
                    else:
                        self.stdout.write(f"      ❌ {rendition.filter_spec}: MISSING - {rend_path}")
