    ...
    def handle(self, *args, **options):
        ...
        try:
            ...
            if not support_index:
                self.stdout.write(
                    self.style.ERROR(
                        "Support index page not found. Please create one first."
                    )
                )
                return
        except Exception as e:
            self.stdout.write(self.style.ERROR(f"Error finding support index: {e}"))
            return
        ...
        for q in questions:
            ...
            try:
                ...
                article.summary = (
                    q["answer"][:450] + "..." if len(q["answer"]) > 450 else q["answer"]
                )
                ...
                self.stdout.write(
                    self.style.WARNING(
                        f"Updated support article: {q['question'][:50]}..."
                    )
                )
                ...
                self.stdout.write(
                    self.style.SUCCESS(
                        f"Created support article: {q['question'][:50]}..."
                    )
                )
