...
class Command(BaseCommand):
    help = (
        "Fix media item dates with accurate publication dates and order chronologically"
    )
    ...
    def handle(self, *args, **options):
        ...
        try:
            ...
            for item in media_page.media_items.all():
                if item.external_url in accurate_dates:
                    ...
                    if new_date and new_date != item.publication_date:
                        ...
                        self.stdout.write(
                            self.style.WARNING(
                                f"  Verification: {date_info['verified']}"
                            )
                        )
                        updated_count += 1
                    else:
            ...
            for index, item in enumerate(
                media_page.media_items.all().order_by("-publication_date")
            ):
                self.stdout.write(
                    self.style.SUCCESS(
