...
class AIContentAnalysisView(View):
    ...
    def post(self, request):
        ...
        try:
            ...
            if not content:
                return JsonResponse(
                    {"error": "No content provided for analysis"}, status=400
                )
            ...
            analysis_result = asyncio.run(
                self.blog_analysis_service.content_processor.analyze_content(
                    content_text, context={"analysis_type": analysis_type, **options}
                )
            )
            ...
            logger.info(
                f"AI content analysis completed for {len(content_text)} characters"
            )
    ...
    def _enhance_analysis_result(
        self, analysis: dict[str, Any], analysis_type: str, options: dict[str, Any]
    ) -> dict[str, Any]:
        ...
        if options.get("include_suggestions", True):
            enhanced["content_suggestions"] = self._generate_content_suggestions(
                analysis
            )
    ...
    def _generate_chartjs_config(self, chart: dict[str, Any]) -> dict[str, Any]:
        ...
        base_config = {
            ...
            "options": {
                "responsive": True,
                "plugins": {
                    "title": {"display": True, "text": chart.get("title", "")},
                    "legend": {"display": True},
                },
            },
        }
                ...
                "datasets": [
                    {
                        "label": "Values",
                        "data": [
                            float(re.sub(r"[^\d.-]", "", stat.get("value", "0")))
