...
def calculate_ytd_return(monthly_returns: Dict[str, Dict[str, str]], current_year: int) -> Tuple[float, float]:
    """
...
def calculate_one_year_return(monthly_returns: Dict[str, Dict[str, str]], 
...
def calculate_three_year_return(monthly_returns: Dict[str, Dict[str, str]], 
                              current_date: date) -> Tuple[float, float]:
    """
    ...
    if len(strategy_returns) >= 36:
        ...
        strategy_annualized = (1 + strategy_compound) ** (1/years) - 1 if years > 0 else 0
    
    if len(benchmark_returns) >= 36:
        benchmark_annualized = (1 + benchmark_compound) ** (1/3) - 1
    else:
        years = len(benchmark_returns) / 12
        benchmark_annualized = (1 + benchmark_compound) ** (1/years) - 1 if years > 0 else 0
    
    return strategy_annualized, benchmark_annualized


def calculate_since_inception_return(monthly_returns: Dict[str, Dict[str, str]], 
                                   inception_date: date,
                                   current_date: date) -> Tuple[float, float]:
    """
    ...
    # Iterate through all years and months since inception
    for year in sorted(monthly_returns.keys()):
        year_int = int(year)
        
        for month_idx, month in enumerate(months):
            # Check if this month is after inception
    ...
    if strategy_returns:
        months_count = len(strategy_returns)
        years = months_count / 12
        strategy_annualized = (1 + strategy_compound) ** (1/years) - 1 if years > 1 else strategy_compound
    else:
        strategy_annualized = 0
    
    if benchmark_returns:
        months_count = len(benchmark_returns)
        years = months_count / 12
        benchmark_annualized = (1 + benchmark_compound) ** (1/years) - 1 if years > 1 else benchmark_compound
    else:
...
def update_performance_from_monthly_data(strategy_page, monthly_returns: Dict[str, Dict[str, str]]):
    """
    ...
    # Calculate 1-year
    one_yr_strategy, one_yr_benchmark = calculate_one_year_return(monthly_returns, current_date)
    ...
    strategy_page.one_year_difference = format_percentage(one_yr_strategy - one_yr_benchmark)
    
    # Calculate 3-year
    three_yr_strategy, three_yr_benchmark = calculate_three_year_return(monthly_returns, current_date)
    ...
    strategy_page.three_year_difference = format_percentage(three_yr_strategy - three_yr_benchmark)
    ...
    if strategy_page.inception_date:
        ...
        strategy_page.since_inception_difference = format_percentage(si_strategy - si_benchmark)