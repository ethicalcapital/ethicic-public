...
def ensure_index_pages():
    ...
    if not encyclopedia_index:
        ...
        if existing_encyclopedia:
            ...
            with connection.cursor() as cursor:
                # Check what columns exist for this table
                cursor.execute("""
                    SELECT column_name 
                    FROM information_schema.columns
                    WHERE table_name = 'public_site_encyclopediaindexpage'
                """)
                columns = [row[0] for row in cursor.fetchall()]
                
                # Build insert based on available columns
                if 'intro_text' in columns:
                    cursor.execute("""
                        INSERT INTO public_site_encyclopediaindexpage (page_ptr_id, intro_text)
                        VALUES (%s, %s)
                        ON CONFLICT (page_ptr_id) DO UPDATE SET
                            intro_text = EXCLUDED.intro_text
                    """, [
                    ...
                    cursor.execute("""
                        INSERT INTO public_site_encyclopediaindexpage (page_ptr_id)
                        VALUES (%s)
