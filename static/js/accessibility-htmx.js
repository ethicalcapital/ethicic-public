...
(function() {
  ...
  const HTMXAccessibility = {
    ...
    setupEventListeners() {
      ...
      document.addEventListener('htmx:timeout', () => {
        LiveRegions.announce('Request timed out. Please check your connection and try again.', true);
      });
    },
    ...
    setupFormValidation() {
      ...
      document.addEventListener('htmx:afterSwap', (event) => {
        ...
        const successElements = target.querySelectorAll('.success, .field-success, .valid-feedback');
    ...
    manageFocusAfterSwap(target) {
      ...
      if (target.id === 'newsletter-response' && target.textContent.includes('success')) {
        const emailInput = document.querySelector('#newsletter-response').closest('.newsletter-widget').querySelector('input[type="email"]');
        if (emailInput) {
          setTimeout(() => {
            emailInput.focus();
          }, 100);
  ...
  const NewsletterAccessibility = {
    ...
    handleNewsletterResponse(target) {
      const isSuccess = target.textContent.toLowerCase().includes('success') ||
